...
module Watson
  describe Config do
    ...
    describe '#create_conf' do
      it 'creates config file, return true' do
    ...
    describe '#read_conf' do
      ...
      context 'when config does exist' do
        ...
        it 'returns true, values match default config' do
          @config.create_conf.should be_true
    ...
    describe '#initialize' do
      it 'initializes all member vars' do
        ...
        @config.github_issues.should == { open: {}, closed: {} }

        @config.bitbucket_valid.should be_false
        @config.bitbucket_api.should == ''
        @config.bitbucket_repo.should == ''
        @config.bitbucket_issues.should == { open: {}, closed: {} }
      end
    end

    describe '#run' do
      it 'populates all member vars' do
        @config.run
        @config.ignore_list.should include('.git', '*.swp')
        @config.tag_list.should include('fix', 'review', 'todo')
        @config.dir_list.should include('.')
