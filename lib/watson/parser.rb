...
module Watson
  ...
  class Parser
    ...
    def get_comment_type(filename)
      ...
      # [review] - It is better for merging each time to stop.
      comment_definitions = COMMENT_DEFINITIONS.merge(@config.type_list)

      pathname = Pathname.new(filename)
      extname = pathname.extname
      debug_print "Extension: #{extname}\n"

      # Break if we don't find a match
      if extname.empty? || !comment_definitions.has_key?(extname)
        # Can't recognize extension, keep looping in case of .bk, .#, ect
        debug_print "Didn't recognize, searching #{pathname.basename}\n"
        false
      else
        comment_definitions[extname]
      end
    end
  end
end
